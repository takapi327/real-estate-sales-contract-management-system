...
class EmployeeTable(tag: Tag) extends SlickTable[Employee](tag, "employee") {
  ...
  /* @4 */ def birthDate     = column[BirthDate]            ("birth_Date")
  ...
  type TableElementTuple = (
    Employee.Id,   NameString,     NameString,  BirthDate,
    AddressString, PhoneNumberInt, EmailString, Option[LicenseNumberString]
  )

  def * = (id, firstName, lastName, birthDate, address, phoneNumber, email, licenseNumber.?) .<> (
    (x: TableElementTuple) => Employee(
      x._1,          FirstName(x._2),   LastName(x._3), x._4,
      Address(x._5), PhoneNumber(x._6), Email(x._7),    x._8.map(LicenseNumber(_))
    ),
    (v: Employee) => Employee.unapply(v).map {t => (
      t._1, t._2.value, t._3.value, t._4, t._5.value, t._6.value, t._7.value, t._8.map(_.value)
    )}
  )
}
